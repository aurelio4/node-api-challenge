...
router.get('/:id', async (req, res) => {
  try {
    ...
    if(!getProjects) {
      res.status(400).json({ error: `No projects found with id:${id}` })
    } else {
...
router.get('/actions/:id', async (req, res) => {
  try {
    ...
    if(!getProjectActions.length) {
      res.status(400).json({ error: "No actions for project" })
    } else {
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const id = req.params.id
    if(name && description) {
      const makeProject = await db.update(id, { name, description })
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    if(deleteProject >= 1) {
      res.status(200).json({ success: `removed ${deleteProject} entries` })
    } else {
      res.status(400).json({ error: `removed ${deleteProject} entries` })
    }
  } catch(err) {
    console.error(err)
    res.status(500).json({ error: "Couldn't fulfill request" })
